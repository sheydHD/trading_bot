...
Sends HTML-formatted emails via Gmail SMTP (TLS on port 587).
Disabled by default; set ``EMAIL_ENABLED=true`` in ``.env``.
...
def send_email(subject: str, content: str, recipient: str | None = None) -> bool:
    """Send an HTML email via Gmail SMTP.

    The ``content`` string is wrapped in a minimal HTML template with
    a heading, pre-wrapped body, and timestamp footer.

    Args:
        subject: Email subject line.
        content: Body text (plain text; rendered inside ``<pre>`` style).
        recipient: Override recipient address.  Falls back to
            ``EMAIL_RECIPIENT`` env var.

    Returns:
        ``True`` if the email was sent successfully, ``False`` otherwise
        (including when email is disabled).
    """
