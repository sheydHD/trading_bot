"""Current-price fetching with Yahoo Finance primary and TradingView fallback."""

import logging
import yfinance as yf

def get_current_price(symbol: str, asset_type: str, tv_indicators=None) -> float | None:
    """Fetch the latest closing price for a symbol.

    Strategy:
        1. Download 1-day daily bar from Yahoo Finance.
        2. If unavailable, fall back to the TradingView ``close``
           indicator (if *tv_indicators* is provided).

    Args:
        symbol: Ticker symbol (e.g. ``"AAPL"``, ``"BTCUSDT"``).
        asset_type: ``"stock"`` or ``"crypto"`` — determines Yahoo
            symbol conversion (e.g. ``BTCUSDT`` → ``BTC-USD``).
        tv_indicators: Optional dict of TradingView indicators;
            used as fallback price source.

    Returns:
        Latest closing price as a float, or ``None`` on failure.
    """
    try:
